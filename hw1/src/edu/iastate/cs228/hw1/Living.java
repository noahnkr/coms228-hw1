...
public abstract class Living 
{
	...
	protected void census(int population[])
	{
		...
		{
			for(int curCol = column - 1; curCol <= column + 1; curCol++)
			{
				if(!(curCol < 0 || curRow < 0 || curCol > plain.getWidth() - 1 || curRow > plain.getWidth() - 1))
				{
					State state = plain.getGrid()[curCol][curRow].who();
